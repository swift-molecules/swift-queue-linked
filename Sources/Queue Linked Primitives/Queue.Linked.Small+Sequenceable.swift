...
// member in the type module (Queue.Linked.Small+Sequenceable.swift) per [MOD-036] refined-C;
// this conformance is thin and splits the `Iterator` associated-type binding from `Iterable`'s
// via `@_implements`. `Queue.Linked.Small` is unconditionally `~Copyable`, so it never conformed
// to `Swift.Sequence`; it now exposes span-primitive iteration via Iterable + Sequenceable.

extension Queue.Linked.Small: Sequenceable where Element: Copyable {
