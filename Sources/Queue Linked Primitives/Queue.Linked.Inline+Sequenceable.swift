...
// member in the type module (Queue.Linked.Inline+Sequenceable.swift) per [MOD-036] refined-C;
// this conformance is thin and splits the `Iterator` associated-type binding from `Iterable`'s
// via `@_implements`. `Queue.Linked.Inline` is unconditionally `~Copyable`, so it never conformed
// to `Swift.Sequence`; it now exposes span-primitive iteration via Iterable + Sequenceable.

extension Queue.Linked.Inline: Sequenceable where Element: Copyable {
