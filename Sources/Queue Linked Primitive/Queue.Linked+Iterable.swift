...
//
// The pointer-chained queue type has NO contiguous element span, so — unlike the contiguous
// single-element containers (set-ordered / stack / heap) which vend `Iterator.Chunk` over a
// `Memory.Contiguous.Protocol` span — `Queue.Linked` produces its bulk iterator by wrapping
// the scalar node-walk `Iterator` in `Iterator.Materializing`, the span-primitive adapter for
// generator-style sequences (the same shape `Single` / dict-ordered / list-linked use). The
// queue therefore does NOT conform `Memory.Contiguous.Protocol` (no element span exists).
//
...
// binds to the scalar `Iterator` (Queue.Linked+Sequenceable.swift).
//
